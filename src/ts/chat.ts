...
const socket: any = io()
var chatUsersTable: any = document.getElementById('chatUsers')
var chatContentsTable: any = document.getElementById('chatContents')
...
const setSelectedChatUser = function(selectedChatUser: string) {
    user.selected = selectedChatUser
}

const getQueryVariable = function(variable: any) {
...
socket.on('getChatUsers', (chatUsers: null) => {
...
const displayGetChatUsersTable = function(i: number, chatUsers: string | any[]) {
...
function chatUserClick(counterpart: string) {
    setSelectedChatUser(counterpart)
    socket.emit('getChatContents', counterpart, getCookie('current-user'))
}

socket.on('displayChatContents', (array: any) => {
    chatContentsTable.innerHTML = '<tbody></tbody>'
    displayChatContentsTable(0, array)
})

socket.on('updateChatContents', (array: any) => {
    chatContentsTable.innerHTML = '<tbody></tbody>'
    displayChatContentsTable(0, array)
})

const displayChatContentsTable = function(i: number, chatContents: string | any[]) {
...
socket.on('declineSendChatContents', (val: number, toastr?: any) => {
    if (val === 0) {
