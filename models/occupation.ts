...
export const initializeOccupationsTable = function () {
    connection.connect(function (err: any) {
        if (err) throw err
        connection.query(`SELECT table_name
            FROM information_schema.tables
            WHERE table_schema = 'coyo'
            AND table_name = 'occupations';`, function (err: any, result: any) {
            if (err) throw err
            if (!result.length) {
                connection.query(`CREATE TABLE IF NOT EXISTS occupations (id INT AUTO_INCREMENT, name VARCHAR(255), PRIMARY KEY (id)) `, function (err: any, result: any) {
                    if (err) throw err
                })
                for (var i = 0; i < occupationsList.length; i++) {
                    connection.query(`INSERT INTO occupations (name) VALUES ("${occupationsList[i]}")`, function (err: any, result: any) {
                        if (err) throw err
                    })
                }
            }
